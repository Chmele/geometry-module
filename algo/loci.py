...
class Loci:
    ...
    def append_point(self, point):
        ...
        self.repr.update({p: self.query(p) + 1 for p in self.get_dominating_points(point, 0)})
        self.repr.update({p: self.query(p) + 1 for p in self.get_dominating_points(point, 1)})
    ...
    def get_dominating_points(self, point, dimension):
        dim_coords = (i.coords[dimension] for i in self.repr if point.coords[dimension] < i.coords[dimension])
    ...
    def get_points_in_rect(self, rect):
        x, y = rect
        q = self.query
        return q(Point(x[1], y[1])) - q(Point(x[0], y[1])) - q(Point(x[1], y[0])) + q(Point(x[0], y[0]))